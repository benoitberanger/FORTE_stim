function varargout = gui_FORTE
% gui_FORTE is the function that creates (or bring to focus) FORTE GUI.
% Then, main_FORTE is always called to start each task. It is the
% "main" program.
...
if ~isempty(figPtr) % Figure exists so brings it to the focus
    ...
    b_FORTE.x   = p_task.xpos(p_task.count);
    b_FORTE.w   = p_task.xwidth(p_task.count);
    b_FORTE.y   = 0.05;
    b_FORTE.h   = 0.90;
    b_FORTE.tag = 'pushbutton_FORTE';
    handles.(b_FORTE.tag) = uicontrol(handles.uipanel_Task,...
        'Style','pushbutton',...
        'Units', 'Normalized',...
        'Position',[b_FORTE.x b_FORTE.y b_FORTE.w b_FORTE.h],...
        'String','FORTE',...
        'BackgroundColor',buttonBGcolor,...
        'Tag',b_FORTE.tag,...
...
% -------------------------------------------------------------------------
function pushbutton_FORTE_Callback(hObject, ~)
handles = guidata(hObject);
cd(handles.path);
main_FORTE(hObject);
end % function
